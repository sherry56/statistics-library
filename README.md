...
| `js/resources.js` | 33 条资料资源及项目内相对路径，包含站点发布/更新日期和可用性标记 |
...
`updatedAt` 表示资料在本站发布或刷新到资料库的日期，并用 `updatedSource: "site-update"` 标明来源；不再读取原文件创建/修改时间。本次 33 条资料（包括 5 个暂不可用占位条目）统一标记为 2026-09-20（今天），不可用状态仍由 `available: false` 控制。`LibraryCore.recent()` 排除缺失/无效日期，按时间倒序取 5 条。后续新增或替换资料时，在数据文件中填写对应的站点发布/更新日期即可。
